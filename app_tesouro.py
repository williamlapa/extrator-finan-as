...
st.image("tesouro_direto.jpeg", caption="Tesouro Direto", use_container_width=True)
...
if df is not None:
    ...
    if all(col in df.columns for col in required_columns):
        ...
        if len(recent_dates) == 2:
            ...
            # Adicionar cor para variação positiva e negativa
            def color_variation(val):
                if val > 0:
                    return 'background-color: #d4edda; color: #155724;'  # Verde para positivo
                elif val < 0:
                    return 'background-color: #f8d7da; color: #721c24;'  # Vermelho para negativo
                else:
                    return ''
            
            styled_df = display_df.style.applymap(color_variation, subset=["Variação (%)"])
            st.dataframe(styled_df)

            # Filtrar grupos específicos
            grupos_interesse = ["Selic", "Prefixado", "IPCA+", "Renda+", "Educa+"]
            for grupo in grupos_interesse:
                st.subheader(f"Gráfico de Variação: {grupo}")
                grupo_df = display_df[display_df["Grupo"].str.contains(grupo, case=False)]
                if not grupo_df.empty:
                    chart_data = grupo_df.set_index("Grupo")[["Variação (%)"]]
                    st.bar_chart(chart_data)
                else:
                    st.write(f"Não há dados para o grupo {grupo}.")
